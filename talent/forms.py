...
class EditProfileForm(FlaskForm):
    ...
    tel_number = StringField('전화번호',
        validators=[DataRequired(), Regexp(_PHONE_RE, message='전화번호는 숫자 9~11자리')],
...
class ExpertSignupForm(FlaskForm):
    ...
    tel_number = StringField('전화번호',
        validators=[DataRequired(), Regexp(_PHONE_RE)],
...
class FindIdForm(FlaskForm):
    name = StringField('이름',
        ...
        render_kw={'id':'name','placeholder':'이름'})
    email = EmailField('이메일',
        validators=[DataRequired('이메일을 입력해주세요'), Email()],
        render_kw={'id':'email','placeholder':'이메일'})
...
class FindPasswordForm(FlaskForm):
    email = EmailField(
        '가입한 이메일 주소',
        validators=[DataRequired('이메일을 입력해주세요'), Email(message='올바른 이메일 형식이 아닙니다.')],
