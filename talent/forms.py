...
from wtforms import StringField, TextAreaField, FileField, PasswordField, BooleanField, SelectField, SubmitField, IntegerField
...
class SignupForm(FlaskForm):
    userid = StringField('아이디',validators=[DataRequired('ID는 필수입력 항목입니다'),
    ...
    password = PasswordField('비밀번호',validators=[DataRequired('비밀번호는 필수입력 항목입니다'),
                                            Regexp(_PASSWORD_RE, message='영문, 숫자, 특수문자를 포함한 8~20자여야 합니다.')],
                                            render_kw={'id':'password','placeholder':'영문·숫자·특수문자 포함 8~20자'})
    password_confirm = PasswordField('비밀번호 확인',validators=[DataRequired('비밀번호를 다시 입력해주세요'),
                                                        EqualTo('password', message='비밀번호가 일치하지 않습니다.')],
                                                        render_kw={'id':'password_confirm','placeholder':'비밀번호를 다시 입력'})
    name = StringField('이름',validators=[DataRequired('이름을 입력해 주세요'),
                                    Length(min=2, max=10, message='이름은 2~10자')],
                                    render_kw={'id':'name','placeholder':'이름을 입력해주세요'})
    email = EmailField('이메일',validators=[DataRequired('이메일 주소를 입력해주세요'),
    ...
    birth_year  = SelectField('생년', choices=_year_choices(),  coerce=int, validators=[DataRequired()], render_kw={'id':'birth_year'})
    birth_month = SelectField('월',   choices=_month_choices(), coerce=int, validators=[DataRequired()], render_kw={'id':'birth_month'})
    birth_day   = SelectField('일',   choices=_day_choices(),   coerce=int, validators=[DataRequired()], render_kw={'id':'birth_day'})
...
class EditProfileForm(FlaskForm):
    name = StringField('이름', validators=[DataRequired(), Length(min=2, max=10)],render_kw={'id':'name','placeholder':'이름'})
    email = EmailField('이메일', validators=[DataRequired(), Email()], render_kw={'id':'email','placeholder':'이메일'})
    tel_number = StringField('전화번호',validators=[DataRequired(), Regexp(_PHONE_RE, message='전화번호는 숫자 9~11자리')],render_kw={'id':'tel_number','placeholder':'전화번호'})
...
SERVICE_CHOICES  = [('', '서비스를 선택하세요'), ('청소','청소'), ('과외','과외'), ('인테리어','인테리어'), ('디자인','디자인'), ('이사','이사')]
LOCATION_CHOICES = [('', '지역을 선택하세요'), ('서울','서울'), ('경기','경기'), ('부산','부산'), ('대구','대구'), ('기타','기타')]

class ExpertSignupForm(FlaskForm):
    userid = StringField('아이디',validators=[DataRequired(), Regexp(_USERID_RE)],
    ...
    email = EmailField('이메일', validators=[DataRequired(), Email()], render_kw={'id':'email','autocomplete':'email'})
    password = PasswordField('비밀번호',validators=[DataRequired(), Regexp(_PASSWORD_RE)],
                             render_kw={'id':'password','placeholder':'영문·숫자·특수문자 포함 8~20자','autocomplete':'new-password'})
    tel_number = StringField('전화번호',validators=[DataRequired(), Regexp(_PHONE_RE)],
                             render_kw={'id':'tel_number','placeholder':'- 없이 숫자만'})
    birth_year  = SelectField('년도', choices=_year_choices(),  coerce=int, validators=[DataRequired()], render_kw={'id':'birth_year'})
    birth_month = SelectField('월',   choices=_month_choices(), coerce=int, validators=[DataRequired()], render_kw={'id':'birth_month'})
    birth_day   = SelectField('일',   choices=_day_choices(),   coerce=int, validators=[DataRequired()], render_kw={'id':'birth_day'})
    service  = SelectField('제공 서비스', choices=SERVICE_CHOICES, validators=[DataRequired()], render_kw={'id':'service'})
    location = SelectField('지역', choices=LOCATION_CHOICES, validators=[DataRequired()], render_kw={'id':'location'})
...
class FindIdForm(FlaskForm):
    name = StringField('이름',validators=[DataRequired('이름을 입력해주세요')],render_kw={'id':'name','placeholder':'이름'})
    email = EmailField('이메일',validators=[DataRequired('이메일을 입력해주세요'), Email()],render_kw={'id':'email','placeholder':'이메일'})
    submit = SubmitField('아이디 찾기')

class FindPasswordForm(FlaskForm):
    email = EmailField('가입한 이메일 주소',validators=[DataRequired('이메일을 입력해주세요'), Email(message='올바른 이메일 형식이 아닙니다.')],
...
class LoginForm(FlaskForm):
    userid = StringField(
        '아이디',validators=[DataRequired('아이디를 입력해주세요'),
            Regexp(_USERID_RE, message='아이디는 영문/숫자/밑줄 6~20자여야 합니다.')],
        render_kw={'id': 'userid', 'class': 'form-control', 'placeholder': '아이디'})

    password = PasswordField(
        '비밀번호',validators=[DataRequired('비밀번호를 입력해주세요')],
...
class ChangePasswordForm(FlaskForm):
    ...
    confirm_password = PasswordField('비밀번호 확인',validators=[DataRequired('비밀번호 확인을 입력하세요'),
